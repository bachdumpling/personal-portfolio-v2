...
const myPortableTextComponents = {
  ...
  block: {
    ...
    normal: ({ children }) => (
      <p className="dark:text-dark-text text-light-text mb-6 text-justify text-sm md:text-base">
        {children}
      </p>
    ),
  },
  list: {
    bullet: ({ children }) => (
      <ul className="list-disc pl-5 mb-10 text-sm md:text-base">{children}</ul>
    ),
    number: ({ children }) => (
      <ol className="list-decimal pl-5 mb-10 text-sm md:text-base">
        {children}
      </ol>
    ),
